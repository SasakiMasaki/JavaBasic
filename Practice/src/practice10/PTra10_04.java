...
public class PTra10_04 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < users.length; i++) {
			...
			String[] usInfo = line.split(",");

			// ★ 配列にした値を、users[カウント]の各フィールドに代入してください
			// ★ ユーザIDに該当する値が数値でなかった場合は、エラーでプログラムが止まっても良いです
			users[i] = new User();
			users[i].userId = Integer.parseInt(usInfo[0]);
			users[i].userNm = usInfo[1];
			users[i].mail = usInfo[2];
			users[i].password = usInfo[3];
		...
		for(int i = 0; i < users.length; i++) {
			System.out.println((i + 1) + "人目の情報を出力します");
